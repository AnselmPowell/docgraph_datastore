...
class DocumentManagementViewSet(viewsets.ViewSet):
    ...
    def __init__(self, **kwargs):
        ...
        logging.info("[DocumentManagementViewSet] Initialized")

    async def _process_document(self, file_data: Dict, user) -> Dict:
        """Process single document and return metadata"""
        logging.info(f"[_process_document] Processing document for user: {user.email}")
        logging.info(f"[_process_document] Processing document: {file_data['file_name']}")
       ...
       # Check for existing document using model query
        ...
        # Create new document
        logging.info(f"[_process_document] Creating new document: {file_data['file_name']}")
        logging.info(f"[_process_document] file_data['file_id']: {file_data['file_id']}")
        logging.info(f"[_process_document] user: {user}")
        logging.info(f"[_process_document] file_data['file_url']: {file_data['file_url']}")
        # print(f"[_process_document] Creating new document: {file_data['file_name']}")
        try:
            document = await sync_to_async(DocumentMetadata.objects.create)(
                # id= uuid.uuid4(),
            ...
            logging.info(f"[_process_document] Created document: {document}")
        except Exception as e:
            logging.info(f"[_process_document] Processing error for {file_data['file_name']}: {str(e)}")
            raise

        # try:
            
        logging.info("Init summarizer document")
        summarizer = DocumentSummarizer()
        
        total_pages = await sync_to_async(doc_processor.get_total_pages)()
        logging.info(f"[_process_document] Total Pages {total_pages}")
        logging.info(f"[_process_document] Processed {len(sections)} sections")
        ...
        logging.info("Done summarising document")
        # Update document metadata
        for field, value in metadata.items():
            setattr(document, field, value)
        document.reference = reference_data
        ...
        # Store sections
        logging.info("Upload Sections Data")
        for section_data in sections:
            try:
                section = await sync_to_async(DocumentSection.objects.create)(
                    document=document,
                ...
                logging.info(f"[_process_document] Error creating section: {str(e)}")
                continue
        
        logging.info("return document data")
        return {
            'document_id': str(document.id),
            'title': document.title,
            'authors': document.authors,
    ...
    async def _upload_documents(self, request):
        logging.info(f"[_upload_documents] User: {request.user.email}")
        logging.info("[_upload_documents] Starting document upload")
        logging.info("[_upload_documents] request data", request.data)
    ...
    def get_documents(self, request):
        ...
        try:
            ...
            # Filter documents by user
            logging.info(f"[get_documents] Fetching for user: {request.user.email}")
            documents = DocumentMetadata.objects.filter(
                user=request.user
            ).order_by('-created_at')
    ...
    def delete_documents(self, request):
        """Delete document and all related data"""
        logging.info("[DELETE] Starting document deletion")
        try:
            document_id = request.data.get('document_id')
            logging.info(f"[DELETE] Attempting to delete document: {document_id}")
            ...
            logging.info(f"[DELETE] Critical error: {str(e)}")
            return Response({
                'status': 'error',
                'message': 'Unable to delete document',
                'detail': 'Please try again or contact support if the problem persists'
